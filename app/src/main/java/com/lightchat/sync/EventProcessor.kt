...
import com.lightchat.notification.NotificationHelper
...
class EventProcessor(
    ...
    private fun handleNewMessage(e: SyncEvent) {
        ...
        if (existing != null) {
            val syncedStatus = if (msg.senderId == app.userSession.currentUserId) {
                MessageStatus.SENT
            } else {
        ...
        val isActiveConversation = app.isAppForeground && app.currentOpenConversationId == msg.conversationId
        ...
        if (conv == null) {
            ...
            conversationDao.insert(
                Conversation(
                    ...
                    unreadCount = if (isActiveConversation || msg.senderId == app.userSession.currentUserId) 0 else 1,
                    atMe = !isActiveConversation && msg.senderId != app.userSession.currentUserId && isMentioned,
                    atMeCount = if (!isActiveConversation && msg.senderId != app.userSession.currentUserId && isMentioned) 1 else 0
                )
            )
        } else {
            ...
            if (msg.senderId != app.userSession.currentUserId && !isActiveConversation) {
        ...
        if (!app.isAppForeground && msg.senderId != app.userSession.currentUserId && !isActiveConversation && notificationConversation?.mute != true) {
            ...
            NotificationHelper.showMessage(
                ...
                notificationTitle,
                notificationContent,
                targetMessageId = if (isGroupMessage && isMentioned) msg.messageId else ""
            )
        }
    }
    ...
    private fun handleMessageRead(e: SyncEvent) {
        ...
        app.groupDao.upsertConversationMemberRead(convId, userId, lastReadSeq)
        if (userId == app.userSession.currentUserId) {
    ...
    private fun handleFriendRequest(e: SyncEvent) {
        ...
        val cv = android.content.ContentValues().apply {
            put("owner_user_id", app.databaseHelper.currentOwnerId())
        ...
        app.databaseHelper.writableDatabase.insert("friend_request", null, cv)
    ...
    private fun handleFriendAccepted(e: SyncEvent) {
        ...
        val db = app.databaseHelper.writableDatabase
        ...
        db.update("friend_request", cv, "owner_user_id = ? AND from_user_id = ? AND to_user_id = ?",
            arrayOf(app.databaseHelper.currentOwnerId(), fromUserId, toUserId))
        AppEvents.notifyConversationChanged(convId)
        AppEvents.notifyFriendRequestChanged()
    }
