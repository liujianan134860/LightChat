...
class SyncManager(
    ...
    private val onNewEventNotify: (Long) -> Unit = { latestUserSeq ->
        if (started) {
            val localSeq = syncStateDao.getLastUserSeq()
        ...
        if (started) {
            ...
            parseAndProcess(json) {
                ...
                if (hasMore && nextSeq > 0L) {
                    ...
                    pendingSync = false
                    val seq = pendingSinceSeq.takeIf { it > 0 } ?: syncStateDao.getLastUserSeq()
                    pendingSinceSeq = 0L
                    triggerSync(seq)
                }
            }
        ...
        if (started) {
            scope.launch {
                ...
                if (msg != null) {
                    ...
                    messageDao.updateStatusAndConversationSeq(messageId, newStatus, conversationSeq)
    ...
    private val onReadNotify: (String, String, Long) -> Unit = { conversationId, readUserId, lastReadSeq ->
        if (started && readUserId != userSession.currentUserId) {
            scope.launch {
    ...
    private fun applyReadReceipt(conversationId: String, readUserId: String, lastReadSeq: Long) {
        ...
        groupDao.upsertConversationMemberRead(conversationId, readUserId, lastReadSeq)
        val updated = messageDao.markLastSentMessageRead(conversationId, currentUserId, lastReadSeq)
        if (updated == 0) {
            messageDao.getLatestSentMessage(conversationId, currentUserId)?.let { latest ->
                messageDao.updateStatus(latest.messageId, com.lightchat.model.MessageStatus.READ)
            }
        }
    ...
    fun requestSync() {
        val seq = if (conversationDao.getVisibleCount() == 0) {
            0L
        } else {
            syncStateDao.getLastUserSeq()
        }
        triggerSync(seq)
    }
